...
class MemoryModule(nn.Module):
    def __init__(self, n_memory, fea_dim, fea_dim2, shrink_thres=0.0025, device=None, memory_init_embedding=None, phase_type=None, dataset_name=None):
        super(MemoryModule, self).__init__()
        self.n_memory = n_memory
        self.fea_dim = fea_dim  # C(=d_model)
        self.fea_dim2 = fea_dim2
        ...
        # self.U2 = nn.Linear(fea_dim*fea_dim, fea_dim*fea_dim)
        # self.W2 = nn.Linear(fea_dim*fea_dim, fea_dim*fea_dim)
        
        # self.U3 = nn.Linear(fea_dim2*fea_dim2, fea_dim2*fea_dim2)
        # self.W3 = nn.Linear(fea_dim2*fea_dim2, fea_dim2*fea_dim2)
        ...
        if self.memory_init_embedding == None:
            if self.phase_type =='test':
                ...
                # self.mem_inter_attn = F.normalize(torch.rand((self.n_memory, self.fea_dim*self.fea_dim), dtype=torch.float), dim=1).cuda()
                # self.mem_time_attn = F.normalize(torch.rand((self.n_memory, self.fea_dim2*self.fea_dim2), dtype=torch.float), dim=1).cuda()
        else:
            # second train 
    ...
    def update_inter_attn(self, query):
        ...
        # update gate : M x C
        update_gate = torch.sigmoid(self.U2(self.mem_inter_attn) + self.W2(add_mem_inter_attn)) # M x C # 这个线性变换是否真的有用？
        self.mem_inter_attn = (1 - update_gate)*self.mem_inter_attn + update_gate*add_mem_inter_attn # 这个门控是否真的有用？
