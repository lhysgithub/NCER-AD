...
def get_dataset_np(dataset):
    if "HADES" in dataset:
        ...
        dim = 50
        train_df = pd.read_csv(f'../MEMTO/data/SWAT/A1_A2/train.csv', sep=",", header=None, skiprows=1,dtype=np.float32).fillna(0)
        test_df = pd.read_csv(f'../MEMTO/data/SWAT/A1_A2/test.csv', sep=",", header=None, skiprows=1,
                              dtype=np.float32).fillna(0)
