...
class GraphAttentionLayerOriginalv2(nn.Module):
    """
    Simple GAT layer, similar to https://arxiv.org/abs/1710.10903
    """
    def __init__(self, in_features, out_features, d_gat, dropout, alpha, concat=True,n_heads=1):
        super(GraphAttentionLayerOriginalv2, self).__init__()
        # self.dropout = dropout
        self.in_features = in_features
        self.out_features = out_features
        self.alpha = alpha
        ...
        self.W = nn.Linear(2*in_features,d_gat)
        self.a = nn.Linear(d_gat,1)
        self.conv1 = nn.Conv1d(in_channels=out_features, out_channels=out_features*4, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=out_features*4, out_channels=out_features, kernel_size=1)
        self.dropout = nn.Dropout(dropout)
        self.activation = F.relu
    ...
    def forward(self, h, adj=None):
        # Wh = self.W(h) # h: B N d_model     adj: B N N
        Wh = h # h: B N d_model     adj: B N N
        e, edges = self._prepare_attentional_mechanism_input(Wh)
        ...
        return self.norm2(x + y), edges

    def _prepare_attentional_mechanism_input(self, Wh):
        B,N,d_model = Wh.shape
        wh1 = Wh.repeat(1,N,1)
        wh2 = Wh.repeat(N,1,1).reshape(B,N*N,d_model)
        edges = torch.cat([wh1,wh2],dim=-1)
        edges = self.leakyrelu(self.W(edges))
        e = self.a(edges).reshape(B,N,N)
        return e, edges
